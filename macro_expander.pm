...
my @tokens = qw(\+ - \* / <= >= < > ; = != [0-9]+\| RANK\| AND OR NOT [()] \.[\d]+ [\d]+[\.]{0,1}[\d]* , CURRENT_RATIO MIN[VOHLC] CDL_BULL_MARUBOZU CMO
                ...
                PPO FOR_TICKER[\s]+[^;]{1,5} KELTNER_LOWER KELTNER_UPPER MFI WMA[VOHLC] STD_DEV ROA REV_PERSHARE PROFIT_MARGIN
                ...
                SHARES_OUTSTANDING RAND DATE_MAX_C DATE_MIN_C
);
...
my %arg_macro_table = ( "V" => "fetch_volume_at", "L" => "fetch_low_at", "MAXO" => "max_open", "MAXV" => "max_volume", 
			...
			"DATE_MAX_C" => "max_close_date", "DATE_MIN_C" => "min_close_date"
);
...
my %noarg_macro_table = ( "=" => "==", "OR" => "||", "AND" => "&&", "BOP" => "compute_bop()", "OBV" => "compute_obv", 
			  ...
			  "POSITION_SELL_PRICE" => "position_sell_price()", "DAYCHANGE" => "closing_price_change()"
);
...
my %transform_table = ( "FOR_TICKER[\\s]+[^;]{1,5}" => \&process_for_ticker, "[0-9]+\\|" => \&process_days_ago,
    "RANK\\|" => \&process_rank);
